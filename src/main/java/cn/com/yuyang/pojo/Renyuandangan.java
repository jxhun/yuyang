package cn.com.yuyang.pojo;


public class Renyuandangan {

    private long id;
    private String xingMing;
    private java.sql.Date ruZhiShiJian;
    private java.sql.Date heTongDaoQiShiJian;
    private String xingBie;
    private java.sql.Date shengRi;
    private String jinJiLianXiDianHua;
    private String xianZhuZhi;
    private String xueLi;
    private String youXiang;
    private String biYeYuanXiao;
    private long zaiZhiZhuangTai;
    private String minZu;
    private String jiGuan;
    private String shenFenZheng;
    private String zhengZhiMianMao;
    private String hunYin;
    private long nianLing;
    private String beiZhu;
    private long zhiWuId;
    private long buMenId;
    private String touXiang;
    private Zhiwubiao zhiwubiao;
    private Denglu denglu;


    public Denglu getDenglu() {
        return denglu;
    }

    public void setDenglu(Denglu denglu) {
        this.denglu = denglu;
    }

    private Bumen bumen;

    public Bumen getBumen() {
        return bumen;
    }

    public void setBumen(Bumen bumen) {
        this.bumen = bumen;
    }


    public Zhiwubiao getZhiwubiao() {
        return zhiwubiao;
    }

    public void setZhiwubiao(Zhiwubiao zhiwubiao) {
        this.zhiwubiao = zhiwubiao;
    }

    public long getId() {
        return id;
    }

    public void setId(long id) {
        this.id = id;
    }


    public String getXingMing() {
        return xingMing;
    }

    public void setXingMing(String xingMing) {
        this.xingMing = xingMing;
    }


    public java.sql.Date getRuZhiShiJian() {
        return ruZhiShiJian;
    }

    public void setRuZhiShiJian(java.sql.Date ruZhiShiJian) {
        this.ruZhiShiJian = ruZhiShiJian;
    }


    public java.sql.Date getHeTongDaoQiShiJian() {
        return heTongDaoQiShiJian;
    }

    public void setHeTongDaoQiShiJian(java.sql.Date heTongDaoQiShiJian) {
        this.heTongDaoQiShiJian = heTongDaoQiShiJian;
    }


    public String getXingBie() {
        return xingBie;
    }

    public void setXingBie(String xingBie) {
        this.xingBie = xingBie;
    }


    public java.sql.Date getShengRi() {
        return shengRi;
    }

    public void setShengRi(java.sql.Date shengRi) {
        this.shengRi = shengRi;
    }


    public String getJinJiLianXiDianHua() {
        return jinJiLianXiDianHua;
    }

    public void setJinJiLianXiDianHua(String jinJiLianXiDianHua) {
        this.jinJiLianXiDianHua = jinJiLianXiDianHua;
    }


    public String getXianZhuZhi() {
        return xianZhuZhi;
    }

    public void setXianZhuZhi(String xianZhuZhi) {
        this.xianZhuZhi = xianZhuZhi;
    }


    public String getXueLi() {
        return xueLi;
    }

    public void setXueLi(String xueLi) {
        this.xueLi = xueLi;
    }


    public String getYouXiang() {
        return youXiang;
    }

    public void setYouXiang(String youXiang) {
        this.youXiang = youXiang;
    }


    public String getBiYeYuanXiao() {
        return biYeYuanXiao;
    }

    public void setBiYeYuanXiao(String biYeYuanXiao) {
        this.biYeYuanXiao = biYeYuanXiao;
    }


    public long getZaiZhiZhuangTai() {
        return zaiZhiZhuangTai;
    }

    public void setZaiZhiZhuangTai(long zaiZhiZhuangTai) {
        this.zaiZhiZhuangTai = zaiZhiZhuangTai;
    }


    public String getMinZu() {
        return minZu;
    }

    public void setMinZu(String minZu) {
        this.minZu = minZu;
    }


    public String getJiGuan() {
        return jiGuan;
    }

    public void setJiGuan(String jiGuan) {
        this.jiGuan = jiGuan;
    }


    public String getShenFenZheng() {
        return shenFenZheng;
    }

    public void setShenFenZheng(String shenFenZheng) {
        this.shenFenZheng = shenFenZheng;
    }


    public String getZhengZhiMianMao() {
        return zhengZhiMianMao;
    }

    public void setZhengZhiMianMao(String zhengZhiMianMao) {
        this.zhengZhiMianMao = zhengZhiMianMao;
    }


    public String getHunYin() {
        return hunYin;
    }

    public void setHunYin(String hunYin) {
        this.hunYin = hunYin;
    }


    public long getNianLing() {
        return nianLing;
    }

    public void setNianLing(long nianLing) {
        this.nianLing = nianLing;
    }


    public String getBeiZhu() {
        return beiZhu;
    }

    public void setBeiZhu(String beiZhu) {
        this.beiZhu = beiZhu;
    }


    public long getZhiWuId() {
        return zhiWuId;
    }

    public void setZhiWuId(long zhiWuId) {
        this.zhiWuId = zhiWuId;
    }


    public long getBuMenId() {
        return buMenId;
    }

    public void setBuMenId(long buMenId) {
        this.buMenId = buMenId;
    }


    public String getTouXiang() {
        return touXiang;
    }

    public void setTouXiang(String touXiang) {
        this.touXiang = touXiang;
    }

}
